...
FIG_name = f'featureimportance_{timestamp}_Total_price.png'
lift_curve_name = f'liftcurve_{timestamp}_Total_price.png'
...
r2, mae, mape, mse , scores_mae,scores_mape = evaluate_partitions(gbm_Flag_weight, df = test_Flag_ans, pred_column = 'y_predict', ans_column = 'Y_valid',partitions = default_partitions,index_prefix ='valid' )

if MLFLOW:
    mlflow.log_metrics(
        {'valid-R-square': r2, 'valid-MAE': mae, 'valid-MSE': mse})
...
if MLFLOW:
    mlflow.log_metrics(
        {'test-R-square': r2, 'test-MAE': mae, 'test-MSE': mse})
...
MODEL_NAME = f'model_{timestamp}_Total_price.pkl'
